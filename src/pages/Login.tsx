...
import { Lock, Mail, ArrowRight, Star, Sparkles } from 'lucide-react'
...
export default function Login() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success('Bon retour parmi nous ! 🌸', { icon: '🎀' })
      navigate('/dashboard')
    },
    onError: (error: any) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-20 px-4 relative overflow-hidden">
      ...
      <motion.div
        ...
        className="max-w-xl w-full bg-white/40 backdrop-blur-2xl rounded-[4rem] p-10 md:p-14 border border-white relative overflow-hidden shadow-2xl group"
      >
        ...
        <div className="text-center mb-12 relative z-10">
          ...
          >
            ...
            <span className="text-[10px] font-black text-pink-deep uppercase tracking-[0.2em]">Accès Membre</span>
          </motion.div>
          <h2 className="text-4xl md:text-5xl font-black text-hello-black tracking-tight leading-tight mb-4">
            Bon <span className="text-pink-deep italic font-serif font-normal">Retour</span>
          </h2>
          <p className="text-lg text-hello-black/40 font-medium italic leading-relaxed">
            "Prête pour une session productive ?" ✨
          </p>
        </div>

        <form onSubmit={handleLogin} className="space-y-8 relative z-10">
          <div className="space-y-4">
            <label className="text-[10px] font-bold text-hello-black/30 uppercase tracking-[0.3em] ml-6">Email</label>
          ...
          <div className="space-y-4">
            <label className="text-[10px] font-bold text-hello-black/30 uppercase tracking-[0.3em] ml-6">Mot de passe</label>
            <div className="relative group/input">
              <div className="absolute left-6 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-2xl flex items-center justify-center border border-pink-milk/10 shadow-sm z-10 group-focus-within/input:scale-110 transition-transform">
          ...
          <div className="pt-4 space-y-8">
            ...
            <div className="flex items-center gap-6">
              ...
              <span className="text-[9px] font-black text-hello-black/20 uppercase tracking-[0.4em]">OU</span>
              <div className="h-[1px] flex-1 bg-pink-milk/20" />
            </div>

            <button
              type="button"
              onClick={() => toast.error('Google Login sera bientôt disponible ! 🌸')}
        ...
        <div className="mt-12 text-center pt-8 border-t border-pink-milk/10">
          <p className="text-hello-black/40 text-sm font-medium italic">
            ...
            <Link to="/auth/register" className="text-pink-deep font-black uppercase tracking-[0.2em] text-[10px] ml-3 hover:text-pink-candy transition-colors relative group/link">
              Créer un carnet ✨
